...
// .wrangler/tmp/bundle-aAkCFN/checked-fetch.js
...
// .wrangler/tmp/bundle-aAkCFN/strip-cf-connecting-ip-header.js
...
app.use("*", async (c, next) => {
  const dbUrl = c.env.DATABASE_URL || "postgresql://postgres:postgres@localhost:5432/pump_erp";
...
api.use("*", async (c, next) => {
  ...
  try {
    ...
    if (token.startsWith("mock-")) {
      ...
      const orgs = await db.select().from(schema_exports.organizations).limit(1);
      let orgId = orgs[0]?.id;
      if (!orgId) {
      ...
      let userRec = await db.query.users.findFirst({
      ...
      const assigns2 = await db.select().from(schema_exports.userStationAssignments).where(eq(schema_exports.userStationAssignments.userId, userRec.id));
      c.set("user", {
        id: userRec.id,
        email: userRec.email,
        organizationId: orgId,
    ...
    return c.json({ success: false, error: { code: "UNAUTHORIZED", message: "Token validation failed" } }, 401);
  }
});
api.get("/session", (c) => {
  const user = c.var.user;
...
// .wrangler/tmp/bundle-aAkCFN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-aAkCFN/middleware-loader.entry.ts
