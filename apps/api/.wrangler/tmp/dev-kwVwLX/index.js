...
// .wrangler/tmp/bundle-SFwWhn/checked-fetch.js
...
// .wrangler/tmp/bundle-SFwWhn/strip-cf-connecting-ip-header.js
...
__export(schema_exports, {
  ...
  saleItems: () => saleItems,
  sales: () => sales,
  shiftStaffAssignments: () => shiftStaffAssignments,
  shiftTemplates: () => shiftTemplates,
  shifts: () => shifts,
  stations: () => stations,
  stockMovements: () => stockMovements,
  stockVariances: () => stockVariances,
  supplierTransactions: () => supplierTransactions,
  suppliers: () => suppliers,
  syncEvents: () => syncEvents,
  tanks: () => tanks,
  userRoles: () => userRoles,
  userStationAssignments: () => userStationAssignments,
  users: () => users
});
...
_a120 = entityKind;
__publicField(CasingCache, _a120, "CasingCache");
...
_a121 = entityKind;
__publicField(DrizzleError, _a121, "DrizzleError");
...
_a122 = entityKind;
__publicField(TransactionRollbackError, _a122, "TransactionRollbackError");
...
_a123 = entityKind;
__publicField(Relation, _a123, "Relation");
var _a124;
var Relations = class {
  constructor(table, config) {
    this.table = table;
...
_a124 = entityKind;
__publicField(Relations, _a124, "Relations");
var _a125;
...
_a134 = entityKind;
__publicField(PgDeleteBase, _a134, "PgDelete");
...
_a135 = entityKind;
__publicField(PgInsertBuilder, _a135, "PgInsertBuilder");
var _a136;
...
_a136 = entityKind;
__publicField(PgInsertBase, _a136, "PgInsert");
...
_a137 = entityKind;
__publicField(PgRefreshMaterializedView, _a137, "PgRefreshMaterializedView");
...
_a138 = entityKind;
__publicField(PgUpdateBuilder, _a138, "PgUpdateBuilder");
var _a139;
...
_a139 = entityKind;
__publicField(PgUpdateBase, _a139, "PgUpdate");
...
var _PgCountBuilder = class extends SQL {
  ...
  sql;
  token;
  [(_a140 = entityKind, Symbol.toStringTag)] = "PgCountBuilder";
  session;
...
__publicField(PgCountBuilder, _a140, "PgCountBuilder");
...
_a141 = entityKind;
__publicField(RelationalQueryBuilder, _a141, "PgRelationalQueryBuilder");
var _a142;
...
_a142 = entityKind;
__publicField(PgRelationalQuery, _a142, "PgRelationalQuery");
...
__name(PgRaw, "PgRaw");
_a143 = entityKind;
__publicField(PgRaw, _a143, "PgRaw");
...
_a144 = entityKind;
__publicField(PgDatabase, _a144, "PgDatabase");
...
__name(Cache, "Cache");
_a145 = entityKind;
__publicField(Cache, _a145, "Cache");
var _a146;
...
_a146 = entityKind;
__publicField(NoopCache, _a146, "NoopCache");
...
_a147 = entityKind;
__publicField(PgPreparedQuery, _a147, "PgPreparedQuery");
var _a148;
var PgSession = class {
  constructor(dialect) {
    this.dialect = dialect;
...
_a148 = entityKind;
__publicField(PgSession, _a148, "PgSession");
var _a149;
...
_a149 = entityKind;
__publicField(PgTransaction, _a149, "PgTransaction");
...
var documentSequences = pgTable("document_sequences", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").references(() => organizations.id).notNull(),
...
var userStationAssignments = pgTable("user_station_assignments", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: uuid("user_id").references(() => users.id).notNull(),
  stationId: uuid("station_id").references(() => stations.id).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull()
});
var tanks = pgTable("tanks", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").references(() => organizations.id).notNull(),
  stationId: uuid("station_id").references(() => stations.id).notNull(),
  name: varchar("name", { length: 100 }).notNull(),
  productId: uuid("product_id").notNull(),
  // references products table defined below
  capacity: numeric("capacity", { precision: 12, scale: 2 }).notNull(),
  // Liters
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull()
});
var dispenserUnits = pgTable("dispenser_units", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").references(() => organizations.id).notNull(),
  stationId: uuid("station_id").references(() => stations.id).notNull(),
  name: varchar("name", { length: 100 }).notNull(),
...
var products = pgTable("products", {
  ...
  // 'FUEL', 'LUBRICANT', 'ACCESSORY', 'SERVICE'
...
var shiftTemplates = pgTable("shift_templates", {
  id: uuid("id").defaultRandom().primaryKey(),
  organizationId: uuid("organization_id").references(() => organizations.id).notNull(),
  name: varchar("name", { length: 100 }).notNull(),
...
var shiftStaffAssignments = pgTable("shift_staff_assignments", {
  id: uuid("id").defaultRandom().primaryKey(),
...
var nozzleReadings = pgTable("nozzle_readings", {
  id: uuid("id").defaultRandom().primaryKey(),
  shiftId: uuid("shift_id").references(() => shifts.id).notNull(),
...
var customerTransactions = pgTable("customer_transactions", {
  ...
  // 'Credit Sale', 'Collection', 'Adjustment'
...
var stockMovements = pgTable("stock_movements", {
  ...
  // 'Purchase', 'Sale', 'Adjustment', 'Decantation', 'Variance'
  quantity: numeric("quantity", { precision: 12, scale: 3 }).notNull(),
  referenceType: varchar("reference_type", { length: 50 }),
  referenceId: uuid("reference_id"),
  notes: varchar("notes", { length: 500 }),
...
var stockVariances = pgTable("stock_variances", {
  id: uuid("id").defaultRandom().primaryKey(),
  shiftId: uuid("shift_id").references(() => shifts.id).notNull(),
  productId: uuid("product_id").references(() => products.id).notNull(),
...
var dssrSnapshots = pgTable("dssr_snapshots", {
  id: uuid("id").defaultRandom().primaryKey(),
  shiftId: uuid("shift_id").references(() => shifts.id).notNull(),
...
var syncEvents = pgTable("sync_events", {
...
_a150 = entityKind;
__publicField(ConsoleLogWriter, _a150, "ConsoleLogWriter");
var _a151;
...
_a151 = entityKind;
__publicField(DefaultLogger, _a151, "DefaultLogger");
var _a152;
...
_a152 = entityKind;
__publicField(NoopLogger, _a152, "NoopLogger");
...
function createDb(connectionString) {
  const queryClient = src_default(connectionString, { prepare: false });
  return drizzle(queryClient, { schema: schema_exports });
}
...
// ../../packages/shared/dist/schemas/validation.js
var organizationSchema = external_exports.object({
  name: external_exports.string().min(2, "Organization name must be at least 2 characters"),
  subscriptionPlan: external_exports.string().default("Core"),
  subscriptionStatus: external_exports.string().default("Active")
});
var stationSchema = external_exports.object({
  name: external_exports.string().min(2, "Station name must be at least 2 characters"),
  code: external_exports.string().min(2, "Station code must be at least 2 characters").toUpperCase(),
  ...
  onboardingStatus: external_exports.enum(["NOT_STARTED", "IN_PROGRESS", "READY_FOR_OPERATIONS"]).default("NOT_STARTED"),
...
var expenseSchema = external_exports.object({
  ...
  parentExpenseId: external_exports.string().uuid().optional().nullable(),
  adjustmentReason: external_exports.string().optional().nullable(),
  status: external_exports.enum(["ACTIVE", "ADJUSTMENT", "VOIDED"]).default("ACTIVE")
});
...
function isAuthorizedForStation(user, resource) {
  if (user.role === "Owner") {
    return true;
  }
...
__name(canManageProduct, "canManageProduct");
function canManageInfrastructure(role) {
  return role === "Owner" || role === "Manager";
}
__name(canManageInfrastructure, "canManageInfrastructure");
...
__name(checkWriteAccess, "checkWriteAccess");
stationSetupRouter.get("/stations", async (c) => {
  const db = c.var.db;
  const user = c.var.user;
  const results = await db.select().from(schema_exports.stations).where(eq(schema_exports.stations.organizationId, user.organizationId));
  const visible = results.filter(
    (s) => isAuthorizedForStation(user, { organizationId: user.organizationId, stationId: s.id })
  );
...
stationSetupRouter.get("/products", async (c) => {
  const db = c.var.db;
  const user = c.var.user;
  const list = await db.select().from(schema_exports.products).where(eq(schema_exports.products.organizationId, user.organizationId));
...
stationSetupRouter.post("/products", async (c) => {
  ...
  try {
    const body = await c.req.json();
    const [newProduct] = await db.insert(schema_exports.products).values({
      organizationId: user.organizationId,
      name: body.name,
      code: body.code,
...
stationSetupRouter.put("/products/:id", async (c) => {
  ...
  try {
    ...
    const [updated] = await db.update(schema_exports.products).set({
      ...
      and(
        eq(schema_exports.products.id, prodId),
        eq(schema_exports.products.organizationId, user.organizationId)
      )
    ).returning();
    if (!updated) {
...
stationSetupRouter.get("/tanks", async (c) => {
  ...
  const list = await db.select().from(schema_exports.tanks).where(
    and(
      eq(schema_exports.tanks.stationId, stationId),
      eq(schema_exports.tanks.organizationId, user.organizationId)
    )
  );
  return c.json({ success: true, data: list });
});
stationSetupRouter.post("/tanks", async (c) => {
  ...
  try {
    ...
    const [newTank] = await db.insert(schema_exports.tanks).values({
      organizationId: user.organizationId,
...
stationSetupRouter.get("/dispensers", async (c) => {
  ...
  const list = await db.select().from(schema_exports.dispenserUnits).where(
    and(
      eq(schema_exports.dispenserUnits.stationId, stationId),
      eq(schema_exports.dispenserUnits.organizationId, user.organizationId)
    )
  );
  return c.json({ success: true, data: list });
});
stationSetupRouter.post("/dispensers", async (c) => {
  ...
  try {
    ...
    const [du] = await db.insert(schema_exports.dispenserUnits).values({
      organizationId: user.organizationId,
      stationId: body.stationId,
      name: body.name,
      code: body.code,
...
stationSetupRouter.get("/nozzles", async (c) => {
  ...
  const list = await db.select().from(schema_exports.nozzles).where(
    and(
      eq(schema_exports.nozzles.stationId, stationId),
      eq(schema_exports.nozzles.organizationId, user.organizationId)
    )
  );
  return c.json({ success: true, data: list });
});
stationSetupRouter.post("/nozzles", async (c) => {
  ...
  try {
    ...
    const [nozzle] = await db.insert(schema_exports.nozzles).values({
      organizationId: user.organizationId,
      stationId: body.stationId,
      duId: body.duId,
      tankId: body.tankId,
...
stationSetupRouter.get("/shift-templates", async (c) => {
  const db = c.var.db;
  const user = c.var.user;
  const list = await db.select().from(schema_exports.shiftTemplates).where(eq(schema_exports.shiftTemplates.organizationId, user.organizationId));
  return c.json({ success: true, data: list });
});
stationSetupRouter.post("/shift-templates", async (c) => {
  ...
  try {
    const body = await c.req.json();
    const [tpl] = await db.insert(schema_exports.shiftTemplates).values({
      organizationId: user.organizationId,
      name: body.name,
...
stationSetupRouter.get("/users", async (c) => {
  const db = c.var.db;
  const user = c.var.user;
  const userList = await db.select().from(schema_exports.users).where(eq(schema_exports.users.organizationId, user.organizationId));
  const mapped = await Promise.all(
    userList.map(async (u) => {
      const roleRec = await db.query.userRoles.findFirst({
        where: eq(schema_exports.userRoles.userId, u.id)
      });
      const assigns = await db.select().from(schema_exports.userStationAssignments).where(eq(schema_exports.userStationAssignments.userId, u.id));
      return {
        ...u,
...
stationSetupRouter.put("/tanks/:id", async (c) => {
  ...
  try {
    ...
    const [updated] = await db.update(schema_exports.tanks).set({
      name: body.name,
      productId: body.productId,
      capacity: body.capacity,
      updatedAt: /* @__PURE__ */ new Date()
...
stationSetupRouter.delete("/tanks/:id", async (c) => {
  ...
  try {
    ...
    const [deleted] = await db.delete(schema_exports.tanks).where(
      and(
        eq(schema_exports.tanks.id, id),
        eq(schema_exports.tanks.organizationId, user.organizationId)
      )
...
stationSetupRouter.put("/dispensers/:id", async (c) => {
  ...
  try {
    ...
    const [updated] = await db.update(schema_exports.dispenserUnits).set({
      name: body.name,
      code: body.code,
      status: body.status,
      updatedAt: /* @__PURE__ */ new Date()
      ...
      and(
        eq(schema_exports.dispenserUnits.id, id),
        eq(schema_exports.dispenserUnits.organizationId, user.organizationId)
      )
    ).returning();
    if (!updated) {
...
stationSetupRouter.delete("/dispensers/:id", async (c) => {
  ...
  try {
    ...
    const [deleted] = await db.delete(schema_exports.dispenserUnits).where(
      and(
        eq(schema_exports.dispenserUnits.id, id),
        eq(schema_exports.dispenserUnits.organizationId, user.organizationId)
      )
...
stationSetupRouter.put("/nozzles/:id", async (c) => {
  ...
  try {
    ...
    const [updated] = await db.update(schema_exports.nozzles).set({
      duId: body.duId,
      tankId: body.tankId,
      productId: body.productId,
      name: body.name,
      ...
      and(
        eq(schema_exports.nozzles.id, id),
        eq(schema_exports.nozzles.organizationId, user.organizationId)
      )
    ).returning();
    if (!updated) {
...
stationSetupRouter.delete("/nozzles/:id", async (c) => {
  ...
  try {
    ...
    const [deleted] = await db.delete(schema_exports.nozzles).where(
      and(
        eq(schema_exports.nozzles.id, id),
        eq(schema_exports.nozzles.organizationId, user.organizationId)
      )
...
stationSetupRouter.put("/shift-templates/:id", async (c) => {
  ...
  try {
    const body = await c.req.json();
    const [updated] = await db.update(schema_exports.shiftTemplates).set({
      name: body.name,
      startTime: body.startTime,
      endTime: body.endTime,
      isActive: body.isActive
      ...
      and(
        eq(schema_exports.shiftTemplates.id, id),
        eq(schema_exports.shiftTemplates.organizationId, user.organizationId)
      )
    ).returning();
    if (!updated) {
...
stationSetupRouter.delete("/shift-templates/:id", async (c) => {
  ...
  try {
    const [deleted] = await db.delete(schema_exports.shiftTemplates).where(
      and(
        eq(schema_exports.shiftTemplates.id, id),
        eq(schema_exports.shiftTemplates.organizationId, user.organizationId)
      )
...
stationSetupRouter.get("/onboarding/status", async (c) => {
  ...
  try {
    const [station] = await db.select().from(schema_exports.stations).where(
      and(
        eq(schema_exports.stations.id, stationId),
        eq(schema_exports.stations.organizationId, user.organizationId)
      )
    ...
    const prodList = await db.select().from(schema_exports.products).where(eq(schema_exports.products.organizationId, user.organizationId));
    const fuelCount = prodList.filter((p) => p.productType === "FUEL" && p.isActive).length;
    const totalProducts = prodList.length;
    const tankList = await db.select().from(schema_exports.tanks).where(
      and(
        eq(schema_exports.tanks.stationId, stationId),
        eq(schema_exports.tanks.organizationId, user.organizationId)
      )
    ...
    const duList = await db.select().from(schema_exports.dispenserUnits).where(
      and(
        eq(schema_exports.dispenserUnits.stationId, stationId),
        eq(schema_exports.dispenserUnits.organizationId, user.organizationId)
      )
    ...
    const nozzleList = await db.select().from(schema_exports.nozzles).where(
      and(
        eq(schema_exports.nozzles.stationId, stationId),
        eq(schema_exports.nozzles.organizationId, user.organizationId)
      )
    ...
    const templateList = await db.select().from(schema_exports.shiftTemplates).where(eq(schema_exports.shiftTemplates.organizationId, user.organizationId));
...
api.use("*", async (c, next) => {
  ...
  try {
    ...
    if (token.startsWith("mock-")) {
      const mockRole = token.split("-")[1];
      let orgs;
      try {
        orgs = await db.select().from(schema_exports.organizations).limit(1);
      } catch (dbErr) {
        const errDetails = {
      ...
      if (!orgId) {
        try {
          const [newOrg] = await db.insert(schema_exports.organizations).values({
            name: "Demo Org"
          }).returning();
          orgId = newOrg.id;
        } catch (dbErr) {
      ...
      try {
        userRec = await db.query.users.findFirst({
          where: eq(schema_exports.users.email, `mock-${mockRole.toLowerCase()}@pump.com`)
        });
      } catch (dbErr) {
      ...
      let assigns2;
      try {
        assigns2 = await db.select().from(schema_exports.userStationAssignments).where(eq(schema_exports.userStationAssignments.userId, userRec.id));
      } catch (dbErr) {
      ...
      c.set("user", {
        ...
        organizationId: orgId,
        role: mockRole,
        assignedStationIds: assigns2.map((a) => a.stationId)
      });
      return await next();
    }
    const { header, payload: decodedPayload } = decode2(token);
    if (header.alg === "HS256") {
      payload = await verify2(token, secret, "HS256");
      ...
      payload = await verify2(token, publicKey, "ES256");
    } else {
      throw new Error(`Unsupported JWT algorithm: ${header.alg}`);
    }
    const authId = payload.sub;
    const dbUser = await db.query.users.findFirst({
      where: eq(schema_exports.users.authUserId, authId)
    });
    if (!dbUser || dbUser.status === "INACTIVE") {
      return c.json({ success: false, error: { code: "FORBIDDEN", message: "User profile inactive or not found" } }, 403);
    }
    const roleRec = await db.query.userRoles.findFirst({
      where: eq(schema_exports.userRoles.userId, dbUser.id)
    });
    const userRole = roleRec ? roleRec.role : "Staff";
    const assigns = await db.select().from(schema_exports.userStationAssignments).where(eq(schema_exports.userStationAssignments.userId, dbUser.id));
    c.set("user", {
      id: dbUser.id,
      email: dbUser.email,
...
// .wrangler/tmp/bundle-SFwWhn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SFwWhn/middleware-loader.entry.ts
