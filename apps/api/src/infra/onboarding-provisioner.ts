...
export class DrizzleOnboardingProvisioner implements OnboardingProvisioner {
  ...
  async provision(input: {
    ...
    try {
      const existingStation = await this.tx
        .select()
      ...
      const [newStation] = await this.tx
      ...
      for (const product of draft.products) {
        ...
        const [createdProduct] = await this.tx
      ...
      for (const tank of draft.tanks) {
        ...
        const [createdTank] = await this.tx
          .insert(schema.tanks)
          .values({
            organizationId,
      ...
      for (const dispenser of draft.dispensers) {
        const [createdDispenser] = await this.tx
      ...
      for (const nozzle of draft.nozzles) {
        ...
        await this.tx.insert(schema.nozzles).values({
          organizationId,
          stationId: newStation.id,
      ...
      if (pendingOpeningStockSeed.length > 0 && actorId) {
        ...
        const [openingDay] = await this.tx
          ...
          .returning();
        await this.tx.insert(schema.stockMovements).values(
      ...
      if (activeFuelProducts.length > 0) {
        await this.tx.insert(schema.fuelPrices).values(
      ...
      if (draft.shiftTemplates.length > 0) {
        await this.tx.insert(schema.shiftTemplates).values(
      ...
      if (paymentTerminals.length > 0) {
        await this.tx.insert(schema.paymentTerminals).values(
      ...
      const provisioner = new AccountProvisioningService(this.tx);
      ...
      const [readyStation] = await this.tx
