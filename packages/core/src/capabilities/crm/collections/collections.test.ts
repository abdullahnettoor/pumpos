...
describe('RecordCollection (decoupled)', () => {
  it('CASH at counter attaches to shift + drawer', async () => {
    ...
    const result = await new RecordCollection({
      ...
      businessDays: new BdRepo([{ ...bday(), id: 'bd-1' }]), docNumbers, events: new InProcessEventDispatcher({ store }),
  ...
  it('accepts a closed-day collection and marks the record, ledger row, and event as late', async () => {
    ...
    const result = await new RecordCollection({
      collections, ledger, customers: new CustomerRepo([customer()]), shifts: new ShiftRepo([]),
      businessDays: new BdRepo([closedDay]), docNumbers, events: new InProcessEventDispatcher({ store }),
    }).execute({ customerId: 'cust-1', amount: 5000, paymentMethod: 'BankTransfer', stationId: 'st-1' }, ctx());

    expect(result.success).toBe(true);
    expect(collections.rows[0].metadata).toEqual({ lateEntry: true });
    expect(ledger.rows[0].metadata).toEqual({ lateEntry: true });
    expect(store.events[0].metadata).toMatchObject({ lateEntry: true, lateEntryPrimary: true, grouping: { role: 'primary' } });
  });

  it('retains a closed shift on a non-drawer late collection', async () => {
    const collections = new CollRepo();
    const closedDay = { ...bday(), id: 'bd-1', status: 'CLOSED' as const, closedAt: '2026-03-15T09:00:00Z' };
    const closedShift = { ...shift(), status: 'CLOSED' as const, closedAt: '2026-03-15T08:30:00Z', closedBy: 'u' };
    const result = await new RecordCollection({
      collections, ledger: new LedgerRepo(), customers: new CustomerRepo([customer()]), shifts: new ShiftRepo([closedShift]),
      businessDays: new BdRepo([closedDay]), docNumbers, events: new InProcessEventDispatcher({ store: new InMemoryEventStore() }),
    }).execute({ customerId: 'cust-1', amount: 500, paymentMethod: 'Card', shiftId: 'sh-1' }, ctx());
  ...
  it('rejects a cash collection without an open shift', async () => {
    const collections = new CollRepo();
    const result = await new RecordCollection({
      collections, ledger: new LedgerRepo(), customers: new CustomerRepo([customer()]), shifts: new ShiftRepo([]),
      businessDays: new BdRepo([bday()]), docNumbers, events: new InProcessEventDispatcher({ store: new InMemoryEventStore() }),
    }).execute({ customerId: 'cust-1', amount: 500, paymentMethod: 'Cash', stationId: 'st-1' }, ctx());
  ...
  it('rejects a drawer collection against a closed shift and closed day', async () => {
    ...
    const result = await new RecordCollection({
      collections, ledger: new LedgerRepo(), customers: new CustomerRepo([customer()]), shifts: new ShiftRepo([closedShift]),
      businessDays: new BdRepo([closedDay]), docNumbers, events: new InProcessEventDispatcher({ store: new InMemoryEventStore() }),
    }).execute({ customerId: 'cust-1', amount: 500, paymentMethod: 'Cash', shiftId: 'sh-1' }, ctx());
  ...
  it('retains optional shift attribution for a non-drawer collection', async () => {
    ...
    const result = await new RecordCollection({
      ...
      businessDays: new BdRepo([{ ...bday(), id: 'bd-1' }]), docNumbers, events: new InProcessEventDispatcher({ store: new InMemoryEventStore() }),
