...
describe('VoidIncome drawer guard', () => {
  it('refuses to void drawer cash income once its shift is closed', async () => {
    ...
    const result = await new VoidIncome({ income, shifts, events: new InProcessEventDispatcher({ store: new InMemoryEventStore() }) }).execute({ id }, ctx());
...
const category = (taxConfig: Record<string, unknown> | null): IncomeCategory => ({
  id: 'cat-1', organizationId: 'org-1', name: 'Tanker Rental', taxConfig, isSystem: false, isActive: true,
});

describe('computeIncomeTax (FI4)', () => {
  it('extracts GST from a tax-inclusive amount, split CGST+SGST intra-state', () => {
    const t = computeIncomeTax(11800, category({ gst_rate: 18, hsn_code: '996601' }), { supplierStateCode: '32', buyerStateCode: '32' });
  ...
  it('uses IGST when the payer is in another state', () => {
    const t = computeIncomeTax(11800, category({ gst_rate: 18 }), { supplierStateCode: '32', buyerStateCode: '29' });
  ...
  it('adds GST on top when the category is priced tax-exclusive', () => {
    const t = computeIncomeTax(10000, category({ gst_rate: 18, price_inclusive: false }), { supplierStateCode: '32' });
    expect(Number(t.taxableAmount)).toBe(10000);
...
describe('RecordIncome tax capture', () => {
  it('freezes the GST split on the entry', async () => {
    ...
    }).execute({ stationId: 'st-1', categoryId: 'cat-1', amount: 11800, receivedInto: 'BANK', supplierStateCode: '32' }, ctx());
    expect(result.success).toBe(true);
    if (result.success) {
  ...
  it('defaults to a zero split when no category repository is wired', async () => {
    ...
    const result = await new RecordIncome({
      ...
      businessDays: new BdRepo([bday()]),
      events: new InProcessEventDispatcher({ store: new InMemoryEventStore() }),
    }).execute({ stationId: 'st-1', categoryId: 'cat-1', amount: 500, receivedInto: 'BANK' }, ctx());
    expect(result.success).toBe(true);
    if (result.success) {
