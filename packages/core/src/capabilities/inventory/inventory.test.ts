...
class VarianceRepo implements StockVarianceRepository {
  ...
  async existsForShift(shiftId: string) { return this.rows.some((row) => row.shiftId === shiftId); }
}
class TankRepo implements TankRepository {
  ...
  async hasNozzles() { return false; }
  async save() {}
  async deactivateById() { return false; }
  async listByStation(orgId: string, stationId: string) { return this.rows.filter((row) => row.organizationId === orgId && row.stationId === stationId); }
}
...
function tank(overrides: Partial<Tank> = {}): Tank {
  return { id: 'tank-A', organizationId: 'org-1', stationId: 'st-1', name: 'MS Tank', productId: 'pet', capacity: '10000', status: 'ACTIVE', createdAt: '', updatedAt: '', ...overrides };
}
function closedShift(overrides: Partial<Shift> = {}): Shift {
  return {
    id: 'sh-1', organizationId: 'org-1', stationId: 'st-1', businessDayId: 'bd-1', shiftTemplateId: 'template-1',
    status: 'CLOSED', openedBy: 'u', openedAt: '', closedBy: 'u', closedAt: '', lockedAt: null,
    openingCash: '0', closingCash: '0', createdAt: '', updatedAt: '', ...overrides,
  };
}

function stockCountDeps(movements = new MovementRepo({ 'tank-A': 5000 }), tanks = new TankRepo([tank()]), shifts = new ShiftRepo()) {
  const variances = new VarianceRepo();
  const store = new InMemoryEventStore();
  return {
    deps: { movements, variances, tanks, shifts, businessDays: new BdRepo([bday()]), events: new InProcessEventDispatcher({ store }) },
    movements, variances, store,
  };
}
...
describe('RecordStockCount', () => {
  ...
  it('bulk dip below book posts negative variance + Variance movement + TANK_DIP + VARIANCE events', async () => {
    ...
    const result = await new RecordStockCount(deps)
      .execute({ stationId: 'st-1', tankId: 'tank-A', actualQuantity: 4950, shiftId: 'sh-1' }, ctx());
    expect(result.success).toBe(true);
    if (result.success) {
    ...
    expect(variances.rows[0]).toMatchObject({ productId: 'pet', shiftId: 'sh-1', businessDayId: 'bd-1' });
  ...
  it('item count matching book records zero variance and no Variance movement', async () => {
    ...
    const variances = new VarianceRepo();
    const store = new InMemoryEventStore();
    const result = await new RecordStockCount({ movements, variances, tanks: new TankRepo([]), shifts: new ShiftRepo(), businessDays: new BdRepo([bday()]), events: new InProcessEventDispatcher({ store }) })
  ...
  it('rejects Shift attribution outside the Tank Dip Station', async () => {
    const { deps } = stockCountDeps(undefined, undefined, new ShiftRepo([closedShift({ stationId: 'other-station' })]));
    const result = await new RecordStockCount(deps).execute({ stationId: 'st-1', tankId: 'tank-A', actualQuantity: 4950, shiftId: 'sh-1' }, ctx());
  ...
  it('records valid closed-Shift attribution', async () => {
    const { deps, variances } = stockCountDeps(undefined, undefined, new ShiftRepo([closedShift()]));
  ...
  it('uses the attributed closed Shift Business Day when a retry crosses the day boundary', async () => {
    ...
    const result = await new RecordStockCount(deps).execute({ stationId: 'st-1', shiftId: 'sh-1', tankId: 'tank-A', actualQuantity: 5000 }, retryContext);
  ...
  it('rejects a caller-supplied Product for a Tank Dip', async () => {
    ...
    const result = await new RecordStockCount(deps).execute({ stationId: 'st-1', tankId: 'tank-A', productId: 'other-product', actualQuantity: 4950 }, ctx());
  ...
  it('records a zero-Variance Tank Dip without a Stock Movement', async () => {
    const { deps, movements, variances, store } = stockCountDeps();
    const result = await new RecordStockCount(deps).execute({ stationId: 'st-1', tankId: 'tank-A', actualQuantity: 5000 }, ctx());

    expect(result.success).toBe(true);
    expect(variances.rows).toHaveLength(1);
    expect(movements.rows).toHaveLength(0);
    expect(store.events.map((event) => event.eventType)).toEqual([BusinessEvents.TANK_DIP_RECORDED]);
  });

  it('records a positive Tank Dip Variance and preserves its reason', async () => {
    const { deps, movements, variances } = stockCountDeps();
    const result = await new RecordStockCount(deps).execute({ stationId: 'st-1', tankId: 'tank-A', actualQuantity: 5025, reason: 'Delivery meter difference' }, ctx());

    expect(result.success).toBe(true);
    expect(variances.rows[0]).toMatchObject({ varianceQuantity: '25', reason: 'Delivery meter difference', shiftId: null });
  ...
  it('rejects a Station outside the execution context', async () => {
    const { deps } = stockCountDeps();
    const result = await new RecordStockCount(deps).execute({ stationId: 'other-station', tankId: 'tank-A', actualQuantity: 5000 }, ctx());
  ...
  it('resolves the Business Day using the Station clock boundary', async () => {
    ...
    const stationContext = { ...ctx(), clock: new FixedClock(new Date('2026-03-15T00:00:00Z')), timeZone: 'Asia/Kolkata', businessDayStartsAt: '06:00' };

    await new RecordStockCount(deps).execute({ stationId: 'st-1', tankId: 'tank-A', actualQuantity: 5000 }, stationContext);
