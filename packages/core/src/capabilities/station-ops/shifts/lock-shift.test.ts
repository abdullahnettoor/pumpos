...
class DayRepo implements BusinessDayWriteRepository {
  ...
  private row(): BusinessDay {
    return {
      ...
      openedBy: 'user-1',
      openedAt: '',
      closedBy: this.status === 'CLOSED' ? 'user-1' : null,
      closedAt: this.status === 'CLOSED' ? '2026-09-12T10:00:00Z' : null,
      createdAt: '',
      updatedAt: '',
    };
  }
...
const context: ExecutionContext = {
  ...
  actorId: 'user-1',
  correlationId: null,
  clock: new FixedClock(new Date('2026-09-12T11:00:00Z')),
...
describe('LockShift', () => {
  it('cannot lock a shift while its Business Day is open', async () => {
    ...
    const result = await new LockShift({
      shifts,
      businessDays: new DayRepo('OPEN'),
  ...
  it('locks a closed shift after its Business Day closes', async () => {
    ...
    const result = await new LockShift({
      shifts,
      businessDays: new DayRepo('CLOSED'),
      events: new InProcessEventDispatcher({ store: new InMemoryEventStore() }),
    }).execute({ shiftId: shift.id }, context);
