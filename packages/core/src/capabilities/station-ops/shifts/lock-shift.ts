...
export class LockShift implements UseCase<LockShiftCommand, Shift> {
  ...
  async execute(input: LockShiftCommand, ctx: ExecutionContext): Promise<Result<Shift>> {
    ...
    const eligibility = await resolveShiftBusinessDayWrite(this.deps.shifts, this.deps.businessDays, ctx, input.shiftId, 'FINANCIAL');
    ...
    if (businessDay.status === 'OPEN') {
      return err(invariantViolation('A Shift cannot be locked while its Business Day is open', { shiftId: shift.id, businessDayId: businessDay.id }));
    }
