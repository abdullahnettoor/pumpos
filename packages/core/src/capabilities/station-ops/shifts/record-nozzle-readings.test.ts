...
import { BusinessEvents, FixedClock, InMemoryEventStore, InProcessEventDispatcher, SequentialIdGenerator } from '../../../kernel/index.js';
...
function shift(status: Shift['status'] = 'OPEN', overrides: Partial<Shift> = {}): Shift {
  return {
    ...
    shiftTemplateId: 'template-1', status, openedBy: 'manager-1', openedAt: '', closedBy: null,
    closedAt: null, lockedAt: null, openingCash: '0', closingCash: null, createdAt: '', updatedAt: '', ...overrides,
  };
}

class ShiftRepo implements ShiftRepository {
  constructor(private readonly row: Shift, private readonly calls: string[]) {}
  async findById() { this.calls.push('shift-lock'); return this.row; }
  async findByIdWithoutLock() { this.calls.push('shift-discovery'); return this.row; }
  async save() {}
  async findOpenByStation() { return this.row.status === 'OPEN' ? this.row : null; }
  async addStaffAssignments() {}
  async addTerminalLinks() {}
}

class BusinessDayRepo implements BusinessDayWriteRepository {
  constructor(private readonly status: BusinessDay['status'], private readonly calls: string[]) {}
...
class ReadingRepo implements NozzleReadingRepository {
  ...
  async lastClosingByNozzleIds() { return new Map<string, number>(); }
  async saveMany() {}
  async listByShift(): Promise<NozzleReading[]> {
    return [{ id: 'reading-1', shiftId: 'shift-1', nozzleId: 'nozzle-1', openingReading: '100', closingReading: '100', volumeSold: '0', testingVolume: '0', unitPrice: '100', createdAt: '' }];
  }
...
const ctx: ExecutionContext = {
  organizationId: 'org-1', stationId: 'station-1', businessDayId: 'day-1', actorId: 'manager-1', correlationId: 'correlation-1',
  clock: new FixedClock(new Date('2026-09-12T08:00:00.000Z')), ids: new SequentialIdGenerator('id'),
};

function setup(shiftStatus: Shift['status'] = 'OPEN', dayStatus: BusinessDay['status'] = 'OPEN', shiftOverrides: Partial<Shift> = {}) {
  const calls: string[] = [];
...
describe('RecordNozzleReadings', () => {
  it('records closing readings for an open Shift and emits an event', async () => {
    ...
    const result = await useCase.execute({ shiftId: 'shift-1', readings: [{ nozzleId: 'nozzle-1', closingReading: 112 }] }, ctx);

    expect(result).toEqual({ success: true, data: { shiftId: 'shift-1', updated: 1, totalVolume: 12 } });
    expect(readings.updates).toEqual([{ id: 'reading-1', closingReading: '112', volumeSold: '12' }]);
    expect(store.events).toHaveLength(1);
    expect(store.events[0]).toMatchObject({ eventType: BusinessEvents.NOZZLE_READING_RECORDED, aggregateId: 'shift-1' });
  });

  it.each(['CLOSED', 'LOCKED'] as const)('rejects a %s Shift without reading or event effects', async (status) => {
