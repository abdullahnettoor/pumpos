...
class ShiftRepo implements ShiftRepository {
  ...
  async save(shift: Shift) { this.saved = shift; }
  async findOpenByStation() { return this.openShift; }
  async addStaffAssignments() {}
  async addTerminalLinks() {}
}

class BusinessDayRepo implements BusinessDayRepository, BusinessDayLock {
  constructor(readonly row: BusinessDay, private readonly calls?: string[]) {}
  async findById(id: string) {
...
const openDay: BusinessDay = {
  ...
  openedBy: 'user-1', openedAt: '', closedBy: null, closedAt: null, createdAt: '', updatedAt: '',
};
...
describe('ReopenShift', () => {
  it('reopens a closed Shift while its Business Day is open', async () => {
    ...
    expect(shifts.saved).toMatchObject({ status: 'OPEN', closedAt: null, closedBy: null, closingCash: null });
    expect(summaries.deleted).toBe(true);
  });

  it('rejects reopening after the parent Business Day is closed', async () => {
    const closedDay: BusinessDay = { ...openDay, status: 'CLOSED', closedAt: '2026-09-12T09:30:00Z' };
  ...
  it('rejects reopening while another Shift is open at the Station', async () => {
    const otherOpenShift: Shift = { ...shift, id: 'shift-2', status: 'OPEN', closedAt: null, closedBy: null, closingCash: null };
  ...
  it('locks Station, Business Day, then Shift after the unlocked discovery read', async () => {
    ...
    expect(result.success).toBe(true);
    expect(calls.slice(0, 5)).toEqual(['shift-discovery', 'station-lock', 'day-lock', 'shift-lock', 'day-read']);
  });
