...
export class ReopenShift implements UseCase<ReopenShiftCommand, Shift> {
  ...
  async execute(input: ReopenShiftCommand, ctx: ExecutionContext): Promise<Result<Shift>> {
    ...
    if (!discoveredShift || discoveredShift.organizationId !== ctx.organizationId) return err(notFoundError('Shift', input.shiftId));
    ...
    if (!businessDay || businessDay.organizationId !== ctx.organizationId || businessDay.stationId !== shift.stationId) {
      return err(notFoundError('BusinessDay', shift.businessDayId));
    }
    if (businessDay.status === 'CLOSED') {
      return err(invariantViolation('Cannot reopen a Shift after its Business Day is closed', {
        shiftId: shift.id,
        businessDayId: businessDay.id,
      }));
    }
    if (shift.status === 'OPEN') return err(invariantViolation('Shift is already open', { shiftId: shift.id }));
    if (shift.status !== 'CLOSED' && shift.status !== 'LOCKED') return err(invariantViolation('Only closed shifts can be reopened', { shiftId: shift.id, status: shift.status }));
