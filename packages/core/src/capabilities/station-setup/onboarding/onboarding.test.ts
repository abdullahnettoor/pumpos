...
function ctx(): ExecutionContext {
  return {
    ...
    ids: new SequentialIdGenerator('o'),
  };
}
...
function validDraft(): OnboardingDraft {
  return {
    ...
    shiftTemplates: [
      { draftId: 's1', name: 'Day', startTime: '06:00', endTime: '18:00', isActive: true },
    ],
    paymentTerminals: [
      {
        draftId: 'pt1',
...
describe('FinalizeStationOnboarding', () => {
  ...
  it('surfaces a provisioner conflict and emits no event', async () => {
    ...
    const provisioner = new FakeProvisioner(
      err(conflictError('Station code "MS1" already exists')),
    );
    const result = await new FinalizeStationOnboarding({
      provisioner,
      capacity,
