...
export type EventValueFormat = 'text' | 'integer' | 'decimal' | 'inr' | 'date' | 'volume-litres' | 'quantity' | 'percentage';
...
const success = (eventType: BusinessEventType, title: string, templates?: readonly EventActivityTemplateDefinition[]) =>
  definition(eventType, title, 'success', templates);
const info = (eventType: BusinessEventType, title: string, templates?: readonly EventActivityTemplateDefinition[]) =>
  definition(eventType, title, 'info', templates);
const warning = (eventType: BusinessEventType, title: string, templates?: readonly EventActivityTemplateDefinition[]) =>
  definition(eventType, title, 'warning', templates);
const danger = (eventType: BusinessEventType, title: string, templates?: readonly EventActivityTemplateDefinition[]) =>
...
export const EventActivityCatalog = {
  [BusinessEvents.ORGANIZATION_CREATED]: success(BusinessEvents.ORGANIZATION_CREATED, 'Organization created'),
  [BusinessEvents.ORGANIZATION_DEACTIVATED]: danger(BusinessEvents.ORGANIZATION_DEACTIVATED, 'Organization deactivated'),
  [BusinessEvents.ORGANIZATION_REACTIVATED]: success(BusinessEvents.ORGANIZATION_REACTIVATED, 'Organization reactivated'),
  [BusinessEvents.OWNER_INVITE_RESENT]: info(BusinessEvents.OWNER_INVITE_RESENT, 'Owner invitation resent'),
  [BusinessEvents.OWNER_INVITE_REVOKED]: danger(BusinessEvents.OWNER_INVITE_REVOKED, 'Owner invitation revoked'),
  ...
  [BusinessEvents.USER_PASSWORD_RESET]: info(BusinessEvents.USER_PASSWORD_RESET, 'Password reset sent'),
  [BusinessEvents.USER_DEACTIVATED]: danger(BusinessEvents.USER_DEACTIVATED, 'Team member deactivated'),
  [BusinessEvents.USER_REACTIVATED]: success(BusinessEvents.USER_REACTIVATED, 'Team member reactivated'),
  ...
  [BusinessEvents.PAYMENT_TERMINAL_REGISTERED]: success(BusinessEvents.PAYMENT_TERMINAL_REGISTERED, 'Payment terminal added'),
  [BusinessEvents.PAYMENT_TERMINAL_UPDATED]: info(BusinessEvents.PAYMENT_TERMINAL_UPDATED, 'Payment terminal updated'),
  ...
  [BusinessEvents.SHIFT_TEMPLATE_CREATED]: success(BusinessEvents.SHIFT_TEMPLATE_CREATED, 'Shift template added'),
  [BusinessEvents.SHIFT_TEMPLATE_UPDATED]: info(BusinessEvents.SHIFT_TEMPLATE_UPDATED, 'Shift template updated'),
  [BusinessEvents.SHIFT_TEMPLATE_DELETED]: danger(BusinessEvents.SHIFT_TEMPLATE_DELETED, 'Shift template removed'),
  [BusinessEvents.ONBOARDING_COMPLETED]: success(BusinessEvents.ONBOARDING_COMPLETED, 'Station onboarding completed'),
  [BusinessEvents.BUSINESS_DAY_OPENED]: success(BusinessEvents.BUSINESS_DAY_OPENED, 'Business day opened'),
  [BusinessEvents.BUSINESS_DAY_CLOSED]: success(BusinessEvents.BUSINESS_DAY_CLOSED, 'Business day closed'),
  ...
  [BusinessEvents.SHIFT_CLOSED]: success(BusinessEvents.SHIFT_CLOSED, 'Shift closed', [
    template('shift-closed.v1', 'Closed a shift with a cash variance of {cashVariance}.', { cashVariance: 'inr' }),
  ]),
  ...
  [BusinessEvents.ATTENDANT_ASSIGNED]: info(BusinessEvents.ATTENDANT_ASSIGNED, 'Attendant assigned'),
  [BusinessEvents.NOZZLE_READING_RECORDED]: info(BusinessEvents.NOZZLE_READING_RECORDED, 'Nozzle readings recorded'),
  ...
  [BusinessEvents.RETAIL_SALE_CREATED]: success(BusinessEvents.RETAIL_SALE_CREATED, 'Sale recorded', [
    template('retail-sale.amount.v1', 'Recorded a sale for {amount} via {paymentMethod}.', {
      amount: 'inr', paymentMethod: 'text',
    }),
  ]),
  [BusinessEvents.RETAIL_SALE_VOIDED]: danger(BusinessEvents.RETAIL_SALE_VOIDED, 'Sale voided'),
  [BusinessEvents.RETAIL_SALE_RETURNED]: danger(BusinessEvents.RETAIL_SALE_RETURNED, 'Sale returned'),
  [BusinessEvents.DISCOUNT_APPLIED]: warning(BusinessEvents.DISCOUNT_APPLIED, 'Discount applied'),
  [BusinessEvents.STOCK_MOVEMENT_RECORDED]: info(BusinessEvents.STOCK_MOVEMENT_RECORDED, 'Stock movement recorded'),
  ...
  [BusinessEvents.TANK_TRANSFER_COMPLETED]: success(BusinessEvents.TANK_TRANSFER_COMPLETED, 'Tank transfer completed'),
  [BusinessEvents.INVENTORY_ADJUSTED]: warning(BusinessEvents.INVENTORY_ADJUSTED, 'Inventory adjusted'),
  [BusinessEvents.PHYSICAL_COUNT_COMPLETED]: success(BusinessEvents.PHYSICAL_COUNT_COMPLETED, 'Physical count completed'),
  [BusinessEvents.VARIANCE_RECORDED]: warning(BusinessEvents.VARIANCE_RECORDED, 'Variance recorded'),
  ...
  [BusinessEvents.PURCHASE_CREATED]: success(BusinessEvents.PURCHASE_CREATED, 'Purchase recorded', [
    template('purchase.v1', 'Recorded a purchase from {supplierName}: {lineCount} items for {amount}.', {
      supplierName: 'text', lineCount: 'integer', amount: 'inr',
    }),
  ]),
  [BusinessEvents.PURCHASE_APPROVED]: success(BusinessEvents.PURCHASE_APPROVED, 'Purchase approved'),
  [BusinessEvents.GOODS_RECEIVED]: success(BusinessEvents.GOODS_RECEIVED, 'Goods received', [
    template('goods-received.v1', 'Received {quantity} {unit} of {productName} from {supplierName}.', {
      quantity: 'quantity', unit: 'text', productName: 'text', supplierName: 'text',
    }),
  ]),
  [BusinessEvents.SUPPLIER_INVOICE_CREATED]: success(BusinessEvents.SUPPLIER_INVOICE_CREATED, 'Supplier invoice recorded', [
    template('supplier-invoice.v1', 'Recorded invoice {invoiceNumber} from {supplierName} for {amount}.', {
      invoiceNumber: 'text', supplierName: 'text', amount: 'inr',
    }),
  ]),
  [BusinessEvents.SUPPLIER_PAID]: success(BusinessEvents.SUPPLIER_PAID, 'Supplier paid', [
    template('supplier-paid.v1', 'Paid {supplierName} {amount} from {accountName}.', {
      supplierName: 'text', amount: 'inr', accountName: 'text',
    }),
  ]),
  [BusinessEvents.SUPPLIER_OPENING_BALANCE_SET]: info(BusinessEvents.SUPPLIER_OPENING_BALANCE_SET, 'Supplier opening balance set'),
  ...
  [BusinessEvents.CUSTOMER_OPENING_BALANCE_SET]: info(BusinessEvents.CUSTOMER_OPENING_BALANCE_SET, 'Customer opening balance set'),
  ...
  [BusinessEvents.CREDIT_LIMIT_CHANGED]: warning(BusinessEvents.CREDIT_LIMIT_CHANGED, 'Credit limit changed'),
  [BusinessEvents.CREDIT_SALE_CREATED]: success(BusinessEvents.CREDIT_SALE_CREATED, 'Credit sale recorded', [
    template('credit-sale.product.v1', 'Recorded a credit sale to {customerName}: {itemSummary} for {amount}.', {
      customerName: 'text', itemSummary: 'text', amount: 'inr',
    }),
    template('credit-sale.amount-only.v1', 'Recorded a credit sale to {customerName} for {amount}.', {
      customerName: 'text', amount: 'inr',
    }),
  ]),
  [BusinessEvents.CREDIT_SALE_VOIDED]: danger(BusinessEvents.CREDIT_SALE_VOIDED, 'Credit sale voided'),
  [BusinessEvents.CREDIT_PAYMENT_RECEIVED]: success(BusinessEvents.CREDIT_PAYMENT_RECEIVED, 'Credit payment received', [
    template('credit-payment-received.v1', 'Received {amount} from {customerName} via {paymentMethod}.', {
      amount: 'inr', customerName: 'text', paymentMethod: 'text',
    }),
  ]),
  [BusinessEvents.OMC_CARD_SALE_CREATED]: success(BusinessEvents.OMC_CARD_SALE_CREATED, 'OMC card sale recorded'),
  [BusinessEvents.OMC_CARD_SALE_VOIDED]: danger(BusinessEvents.OMC_CARD_SALE_VOIDED, 'OMC card sale voided'),
  ...
  [BusinessEvents.PAYMENT_MADE]: warning(BusinessEvents.PAYMENT_MADE, 'Payment made', [
    template('payment-made.v1', 'Paid {amount} to {partyName} from {accountName}.', {
      amount: 'inr', partyName: 'text', accountName: 'text',
    }),
  ]),
  [BusinessEvents.INVOICE_GENERATED]: success(BusinessEvents.INVOICE_GENERATED, 'Invoice generated'),
  [BusinessEvents.FINANCIAL_ACCOUNT_CREATED]: success(BusinessEvents.FINANCIAL_ACCOUNT_CREATED, 'Financial account added'),
  [BusinessEvents.FINANCIAL_ACCOUNT_UPDATED]: info(BusinessEvents.FINANCIAL_ACCOUNT_UPDATED, 'Financial account updated'),
  [BusinessEvents.LEDGER_ENTRY_POSTED]: info(BusinessEvents.LEDGER_ENTRY_POSTED, 'Ledger entry posted'),
...
function formatValue(value: ActivityValue, format: EventValueFormat): string | null {
  if (format === 'text') return String(value).replace(/[\u0000-\u001F\u007F]/g, ' ').trim() || null;
  const numeric = typeof value === 'number' ? value : Number(value);
  if (!Number.isFinite(numeric)) return null;
  switch (format) {
    case 'integer':
      return Number.isInteger(numeric) ? new Intl.NumberFormat('en-IN', { maximumFractionDigits: 0 }).format(numeric) : null;
    case 'decimal':
    case 'quantity':
    ...
    case 'date': {
      ...
      return Number.isNaN(date.valueOf()) ? null : new Intl.DateTimeFormat('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }).format(date);
    }
    case 'volume-litres':
...
function fallback(eventType: string, definition?: EventActivityDefinition, diagnostic?: RenderedEventActivity['diagnostic']): RenderedEventActivity {
...
export function renderEventActivity(
  ...
  const selected = definition.templates.find((candidate) => candidate.id === metadata.presentation!.templateId);
  if (!selected) return fallback(event.eventType, definition, 'unknown-template');

  let invalidValue = false;
  const description = selected.template.replace(/\{([A-Za-z][A-Za-z0-9]*)\}/g, (match, key: string) => {
    const value = metadata.presentation!.values[key];
    const formatted = value === undefined || value === null ? null : formatValue(value, selected.formats[key]);
    if (formatted === null) {
      invalidValue = true;
      return match;
    }
  ...
  return { title: definition.defaultTitle, description, tone: definition.defaultTone, renderStatus: 'rendered' };
}
