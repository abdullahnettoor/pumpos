...
export const businessDays = pgTable(
  ...
  (t) => ({
    orgStationDateUniq: uniqueIndex('business_days_org_station_date_uniq').on(
      t.organizationId,
      t.stationId,
      t.businessDate,
    ),
...
export const ledgerEntries = pgTable(
  ...
  (t) => ({
    ...
    orgStationDateIdx: index('ledger_entries_org_station_date_idx').on(
      t.organizationId,
      t.stationId,
      t.entryDate,
    ),
...
export const invoices = pgTable(
  ...
  (t) => ({
    ...
    saleUniq: uniqueIndex('invoices_sale_uniq')
      .on(t.saleId)
...
export const events = pgTable(
  ...
  (t) => ({
    activityPrimaryTimelineIdx: index('events_activity_primary_timeline_idx')
      ...
      .where(
        sql`${t.correlationId} IS NULL OR (${t.metadata} -> 'grouping' ->> 'role') = 'primary'`,
      ),
    ...
    activityPrimaryCorrelationUniq: uniqueIndex('events_activity_primary_correlation_uniq')
      ...
      .where(
        sql`${t.correlationId} IS NOT NULL AND (${t.metadata} -> 'grouping' ->> 'role') = 'primary'`,
      ),
  }),
);
...
export const attendantHandovers = pgTable(
  'attendant_handovers',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    organizationId: uuid('organization_id')
    ...
    creditHandedOver: numeric('credit_handed_over', { precision: 12, scale: 2 })
      .default('0')
      .notNull(),
  ...
  (t) => ({
    currentHandoverUniq: uniqueIndex('attendant_handovers_org_station_shift_user_du_uniq').on(
      t.organizationId,
      t.stationId,
      t.shiftId,
      t.userId,
...
export const handoverTerminalEntries = pgTable(
  'handover_terminal_entries',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    organizationId: uuid('organization_id')
      .references(() => organizations.id)
    ...
    handoverId: uuid('handover_id')
      .references(() => attendantHandovers.id, { onDelete: 'cascade' })
      .notNull(),
    shiftId: uuid('shift_id')
      .references(() => shifts.id)
      .notNull(),
