...
const Row: React.FC<{ label: string; children: React.ReactNode; note?: string }> = ({
  label,
  children,
  note,
}) => (
...
const Group: React.FC<{ title: string; description?: string; children: React.ReactNode }> = ({
  title,
  description,
  children,
}) => (
  <section className="mb-10">
    <header className="mb-4">
      <div className="text-[13px] font-semibold uppercase tracking-wider text-ink-strong">
        {title}
      </div>
      {description && (
        <p className="mt-1.5 max-w-[620px] text-[12.5px] leading-relaxed text-ink-muted">
          {description}
        </p>
      )}
    </header>
  ...
  <div>
    {VARIANTS.map((variant) => (
      <Row key={variant} label={`variant=${variant}`}>
        {TONES.map((tone) => (
          <Chip key={tone} tone={tone} variant={variant}>
            {tone}
          </Chip>
        ))}
      </Row>
  ...
  <div>
    {(['xs', 'sm', 'md'] as ChipSize[]).map((size) => (
      <Row key={size} label={`size=${size}`}>
        <Chip size={size} tone="brand" variant="soft">
          soft
        </Chip>
        <Chip size={size} tone="brand" variant="outline">
          outline
        </Chip>
        <Chip size={size} tone="brand" variant="solid">
          solid
        </Chip>
        <Chip size={size} tone="info" icon={<Info />}>
          with icon
        </Chip>
        <Chip size={size} tone="warning" dot pulse>
          with pulse dot
        </Chip>
        <Chip size={size} tone="danger" icon={<ShieldCheck />}>
          hazmat
        </Chip>
      </Row>
    ))}
...
const ChipAdornments: React.FC = () => {
  ...
  return (
    <div>
      <Row label="icon">
        <Chip tone="brand" icon={<Fuel />}>
          HSD
        </Chip>
        <Chip tone="info" icon={<User />}>
          Fleet customer
        </Chip>
        <Chip tone="warning" icon={<Truck />}>
          Fleet supplier
        </Chip>
      ...
      <Row label="dot">
        ...
        <Chip tone="warning" dot pulse>
          Pending
        </Chip>
        <Chip tone="danger" dot pulse>
          Failing
        </Chip>
      ...
      <Row label="removable" note="onRemove wires a close button with its own focus ring">
        ...
        {filters.map((label) => (
          <Chip
            key={label}
            tone="info"
            onRemove={() => setFilters((cs) => cs.filter((c) => c !== label))}
          >
            {label}
          </Chip>
        ))}
    ...
    <div>
      ...
      <Row label="pulse" note="motion-safe:animate-pulse — reduced-motion users get a static dot">
        <span className="inline-flex items-center gap-1.5 text-[11px] text-ink-muted">
          <Dot tone="brand" size="md" pulse /> brand
        </span>
        <span className="inline-flex items-center gap-1.5 text-[11px] text-ink-muted">
          <Dot tone="info" size="md" pulse /> info
        </span>
        <span className="inline-flex items-center gap-1.5 text-[11px] text-ink-muted">
          <Dot tone="success" size="md" pulse /> success
        </span>
        <span className="inline-flex items-center gap-1.5 text-[11px] text-ink-muted">
          <Dot tone="warning" size="md" pulse /> warning
        </span>
        <span className="inline-flex items-center gap-1.5 text-[11px] text-ink-muted">
          <Dot tone="danger" size="md" pulse /> danger
        </span>
      </Row>
    </div>
  );
    ...
    <div>
      ...
      <KpiStrip>
        ...
        <KpiTile
          ...
          hint="7d"
        />
        <KpiTile dot="neutral" label="Pending sync" value="4" hint="retry in 12s" />
      </KpiStrip>
    </div>
    ...
    <div>
      {variants.map((v) => (
        <Row key={v} label={`variant=${v}`}>
          ...
          <Button variant={v} size="sm" leftIcon={<Plus />}>
            New
          </Button>
          <Button variant={v} size="sm" rightIcon={<ArrowRight />}>
            Next
          </Button>
      ...
      <Row label="sizes" note="xs 28 · sm 32 · md 36 · lg 40">
        {sizes.map((s) => (
          <Button key={s} variant="primary" size={s} leftIcon={<Play />}>
            {s.toUpperCase()}
          </Button>
        ))}
      </Row>
      <Row label="icon-only" note="square; always pair with aria-label">
        {sizes.map((s) => (
          <Button key={s} variant="secondary" size={s} iconOnly aria-label="Download">
            <Download />
          </Button>
        ))}
        <Button variant="danger" size="sm" iconOnly aria-label="Delete">
          <Trash2 />
        </Button>
      ...
      <Row label="loading" note="spinner swaps in, width stays stable, clicks blocked">
        ...
        <Button variant="danger" size="sm" loading iconOnly aria-label="Deleting">
          <Trash2 />
        </Button>
      </Row>
      <Row label="full width">
        <div className="w-full max-w-[280px]">
          <Button variant="primary" size="md" fullWidth leftIcon={<Play />}>
            Open shift
          </Button>
        </div>
      </Row>
...
const ButtonInContext: React.FC = () => (
  <div className="space-y-4 py-2">
    <div>
      <div className="mb-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
        drawer footer (one primary action, one secondary)
      </div>
    ...
    <div>
      <div className="mb-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
        destructive confirm (danger primary + safe escape)
      </div>
      <div className="flex items-center justify-end gap-2 rounded-card border border-border-soft bg-surface px-4 py-3">
    ...
    <div>
      <div className="mb-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
        toolbar (xs ghost + icon-only cluster)
      </div>
      <div className="flex items-center gap-1.5 rounded-card border border-border-soft bg-surface px-3 py-2">
        ...
        <Button variant="ghost" size="xs" iconOnly aria-label="Refresh">
          <ArrowRight />
        </Button>
...
const STATUS_GROUPS: { label: string; statuses: PumpStatus[] }[] = [
  {
    label: 'lifecycle',
    statuses: ['open', 'closed', 'locked', 'draft', 'active', 'inactive', 'archived'],
  },
  ...
  <div className="space-y-6 py-2">
        ...
        {[
          {
            id: 'SHIFT-4128',
            operator: 'Rekha M.',
            status: 'open' as PumpStatus,
            sync: 'syncing' as PumpStatus,
            sales: '₹2,84,120',
          },
          {
            id: 'SHIFT-4127',
            operator: 'Anish K.',
            status: 'closed' as PumpStatus,
            sync: 'synced' as PumpStatus,
            sales: '₹4,12,940',
          },
          {
            id: 'SHIFT-4126',
            operator: 'Priya D.',
            status: 'closed' as PumpStatus,
            sync: 'pending' as PumpStatus,
            sales: '₹3,08,220',
          },
          {
            id: 'SHIFT-4125',
            operator: 'Vinod S.',
            status: 'locked' as PumpStatus,
            sync: 'sync-failed' as PumpStatus,
            sales: '₹2,96,410',
          },
        ].map((r) => (
          <div
            key={r.id}
            className="grid grid-cols-[120px_1fr_auto_auto_120px] items-center gap-4 px-4 py-2.5 text-[13px]"
          >
    ...
    <div>
      ...
      <div className="divide-y divide-border-soft rounded-card border border-border-soft bg-surface">
        {[
          {
            name: 'Karnataka State RTC',
            code: 'KSR-041',
            status: 'overdue' as PumpStatus,
            days: '12d',
            balance: '₹12,84,320',
          },
          {
            name: 'Sunrise Logistics Pvt',
            code: 'SUN-018',
            status: 'overdue' as PumpStatus,
            days: '38d',
            balance: '₹9,42,600',
          },
          {
            ...
            code: 'GRF-007',
            status: 'partial' as PumpStatus,
            days: '2d',
            balance: '₹12,450',
          },
          {
            name: 'Metro Cabs Union',
            code: 'MCU-102',
            status: 'settled' as PumpStatus,
            days: '—',
            balance: '₹0',
          },
        ].map((r) => (
          <div
            key={r.code}
            className="grid grid-cols-[1fr_auto_auto_140px] items-center gap-4 px-4 py-2.5 text-[13px]"
          >
            ...
            <StatusChip
              status={r.status}
              size="xs"
              label={r.status === 'overdue' ? `Overdue ${r.days}` : undefined}
            />
...
const DashboardBlocks: React.FC = () => (
  <div className="space-y-6">
    ...
    <div>
      <div className="mb-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
        Panel · titled surface (header + body + footer)
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Panel
          ...
          action={
            <span className="font-mono text-[12px] font-semibold text-ink-strong">₹3,68,540</span>
          }
          footer={
            <span className="text-[11px] text-ink-muted">Cash reconciled at shift close.</span>
          }
        >
          <BreakdownBar
    ...
    <div>
      <div className="mb-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
        MeterRow · auto-tone (critical &lt;5% · low &lt;15% · else brand)
      </div>
    ...
    <div>
      <div className="mb-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
        Sparkline · dependency-free trend (line + optional area fill)
      </div>
      <div className="flex flex-wrap items-center gap-6 rounded-card border border-border-soft bg-surface p-4">
        {[
          ...
          {
            tone: 'success' as DotTone,
            label: 'Volume 7d',
            data: [30, 32, 31, 38, 40, 44, 47],
            fill: true,
          },
    ...
    <div>
      <div className="mb-2.5 font-mono text-[10px] uppercase tracking-wider text-ink-faint">
        EmptyState · compact (inline) + default (block)
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <EmptyState
          compact
