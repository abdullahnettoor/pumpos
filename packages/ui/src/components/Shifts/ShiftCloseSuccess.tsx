...
export interface ShiftCloseSuccessProps {
  ...
  onViewSummary: () => void;
  onBack: () => void;
  tankDips?: Array<{ tankId: string; tankName: string; actualQuantity: number; status: 'pending' | 'saving' | 'saved' | 'failed'; error?: string; expectedQuantity?: number; varianceQuantity?: number }>;
  onSaveTankDips?: () => void;
 ...
 */
export const ShiftCloseSuccess: React.FC<ShiftCloseSuccessProps> = ({ result, onStartNext, onViewSummary, onBack, tankDips = [], onSaveTankDips, onDiscardTankDips }) => {
  const hasPendingDips = tankDips.some((dip) => dip.status === 'pending' || dip.status === 'failed');
  ...
  return (
    <div className="animate-fade-in card card-comfortable" style={{ maxWidth: '600px', margin: '40px auto', display: 'flex', flexDirection: 'column', gap: '24px', textAlign: 'center', fontFamily: 'var(--font-sans)' }}>
      ...
      {tankDips.length > 0 && (
        <div style={{ border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-input)', overflow: 'hidden', textAlign: 'left' }}>
          <div style={{ padding: '10px 14px', backgroundColor: 'var(--bg-surface-alt)', borderBottom: '1px solid var(--border-soft)' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-strong)' }}>Optional Tank Dip</div>
            <div style={{ marginTop: 2, fontSize: 11, color: 'var(--text-muted)' }}>Fuel sales are posted. Save these readings now as a separate Business Day action.</div>
          </div>
          {tankDips.map((dip) => (
            <div key={dip.tankId} style={{ padding: '9px 14px', display: 'flex', justifyContent: 'space-between', gap: 12, borderBottom: '1px solid var(--border-soft)', fontSize: 12 }}>
              <div>
                <div style={{ fontWeight: 600, color: 'var(--text-strong)' }}>{dip.tankName}</div>
                {dip.error && <div style={{ color: 'var(--state-danger-fg)', marginTop: 2 }}>{dip.error}</div>}
              </div>
              <div style={{ textAlign: 'right', fontFamily: 'var(--font-mono)' }}>
                <div>{dip.actualQuantity.toFixed(1)} L</div>
                <div style={{ color: dip.status === 'saved' ? 'var(--state-success-fg)' : dip.status === 'failed' ? 'var(--state-danger-fg)' : 'var(--text-muted)', fontFamily: 'var(--font-sans)' }}>
                  {dip.status === 'saved' ? 'Saved' : dip.status === 'saving' ? 'Saving...' : dip.status === 'failed' ? 'Retry required' : 'Ready'}
                </div>
                {dip.status === 'saved' && dip.expectedQuantity !== undefined && dip.varianceQuantity !== undefined && (
                  <div style={{ marginTop: 2, color: dip.varianceQuantity === 0 ? 'var(--text-muted)' : 'var(--state-warning-fg)', fontFamily: 'var(--font-sans)', fontSize: 11 }}>
                    Expected {dip.expectedQuantity.toFixed(1)} L · Variance {dip.varianceQuantity > 0 ? '+' : ''}{dip.varianceQuantity.toFixed(1)} L
                  </div>
                )}
              </div>
            </div>
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
        <Button variant="primary" size="md" leftIcon={<Play size={13} style={{ fill: 'currentColor' }} />} onClick={onStartNext} disabled={hasPendingDips || isSavingDips}>
          Open next Shift
        </Button>
        <Button variant="secondary" size="md" leftIcon={<FileText size={13} />} onClick={onViewSummary} disabled={hasPendingDips || isSavingDips}>
          View Compiled Shift Summary
        </Button>
        <Button variant="secondary" size="md" onClick={onBack} disabled={hasPendingDips || isSavingDips}>
          Back to Workspace
        </Button>
      </div>
    </div>
