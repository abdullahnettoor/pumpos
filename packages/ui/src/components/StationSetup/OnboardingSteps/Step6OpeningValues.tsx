      ...
      <div style={panelStyle}>
        <div>
          <h2 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--text-strong)' }}>
            Opening / Current Values
          </h2>
          <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px' }}>
            Finalize current rates and opening tank stock before provisioning the station. Nozzle
            opening readings are captured when you open the first operational shift.
          </p>
        </div>
            ...
            {draft.tanks.map((tank) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      style={{ fontWeight: 600, fontSize: '13px', color: 'var(--text-strong)' }}
                    >
                      {tank.name}
                    </span>
                  ...
                  >
                    <span>
                      Capacity: {tank.capacity.toLocaleString()} {product?.unit || 'L'}
                    </span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>
                      Opening Stock ({product?.unit || 'L'})
                    </span>
                    <input
                      type="number"
                      min={0}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>
                      Purchase Rate (₹/{product?.unit || 'L'}) — landed cost
                    </span>
                    <input
                      type="number"
                      min={0}
                    ...
                    <span style={{ fontSize: '9px', color: 'var(--text-faint)' }}>
                      Seeds cost basis for margin. Leave 0 if unknown.
                    </span>
                  </div>
                </div>
              );
