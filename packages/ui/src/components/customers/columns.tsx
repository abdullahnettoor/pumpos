...
export const typeTone = (t: string): 'info' | 'warning' | 'neutral' =>
  t === 'Fleet' ? 'info' : t === 'Credit' ? 'warning' : 'neutral';

/** Payment-method chip tone. */
export const methodTone = (m: string): 'brand' | 'info' | 'warning' | 'neutral' =>
  m === 'UPI'
    ? 'brand'
    : m === 'Card' || m === 'BankTransfer'
      ? 'info'
      : m === 'Credit'
        ? 'warning'
        : 'neutral';
  ...
  const cols: ColumnDef<any, any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (row.original.isPrepaid || limit <= 0)
    ...
    },
    {
      ...
      cell: ({ getValue }) => (
        <StatusChip
          ...
          size="xs"
          label={getValue() ? 'Active' : 'Suspended'}
        />
      ),
    },
    {
  ...
  },
  {
    ...
    cell: ({ getValue }) => (
      <span style={{ fontWeight: 600, color: 'var(--text-strong)' }}>
        {(getValue() as string) || 'Walk-in Customer'}
      </span>
    ),
  },
  {
    ...
    cell: ({ getValue }) => {
      ...
      return m ? (
        <Chip tone={methodTone(m)} size="xs">
          {methodLabel(m)}
        </Chip>
      ) : (
...
export const buildCreditSaleColumns = (): ColumnDef<any, any>[] => [
  ...
  },
  {
    ...
    cell: ({ getValue }) => (
      <span style={{ color: 'var(--text-default)' }}>
        {(getValue() as string) || 'Merchandise'}
      </span>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ getValue }) => (
      <span
        style={{ fontWeight: 700, color: 'var(--brand-warning)', fontFamily: 'var(--font-mono)' }}
      >
        {inr(Number(getValue() || 0))}
      </span>
    ),
...
export const buildVehicleColumns = (
...
): ColumnDef<any, any>[] => [
  {
    ...
    cell: ({ getValue }) => (
      <span
        style={{ fontWeight: 600, color: 'var(--text-strong)', fontFamily: 'var(--font-mono)' }}
      >
        {getValue() as string}
      </span>
    ),
    ...
    cell: ({ row }) => {
      ...
      return (
        ...
        >
          ...
          {v.customerType && (
            <Chip tone={typeTone(v.customerType)} size="xs">
              {v.customerType}
            </Chip>
          )}
        </div>
