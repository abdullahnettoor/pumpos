  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
      <span
        style={{
          fontWeight: 700,
          color: voided ? 'var(--text-faint)' : 'var(--brand-success)',
  ...
  { accessorKey: 'entryDate', header: 'Entry Date', cell: ({ row }) => dateCell(row) },
  {
    ...
    cell: ({ getValue }) => (
      <span style={{ fontWeight: 600, color: 'var(--text-strong)' }}>
        {(getValue() as string) ?? 'Other Income'}
      </span>
    ),
  },
  {
...
/** Full income ledger columns incl. Account — shared by the Ledger tab and the register. */
