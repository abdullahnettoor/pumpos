...
export const LedgerView: React.FC<LedgerViewProps> = ({
  ...
  const {
    ...
    totalCredit,
    closingBalance,
  } = computeLedgerRows(entries, resolve, openingBalance);
    ...
    >
      ...
      >
            ...
            return (
              <tr key={r.id} style={{ borderBottom: '1px solid var(--border-soft)' }}>
                <td style={{ padding: '8px 12px' }}>
                  ...
                  {r.subLabel && (
                    <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{r.subLabel}</div>
                  )}
                </td>
                <td style={{ padding: '8px 12px' }}>
                  <span style={{ fontWeight: 600, color: r.typeColor || 'var(--text-default)' }}>
                    {r.type}
                  </span>
                  {r.notes && (
        ...
        <tfoot>
          ...
          >
            <td
              style={{ padding: '8px 12px', fontWeight: 700, color: 'var(--text-strong)' }}
              colSpan={2}
            >
              Totals
            </td>
