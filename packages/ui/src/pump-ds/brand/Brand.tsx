...
export type PumpOSMarkProps = SVGAttributes<SVGSVGElement>;

/**
 * Mark height within the lockup, relative to the inherited font size.
 *
 * Both sit above 1em on purpose: matched to the cap height, the nozzle cutout
 * closes up into a blob — it takes roughly 1.6em before the detail reads at the
 * topbar's 15px. Stacked, the mark leads rather than accompanies, so it grows.
 */
...
export const PumpOSLockup = forwardRef<HTMLSpanElement, PumpOSLockupProps>(function PumpOSLockup(
  ...
  return (
    ...
    >
      ...
      <PumpOSMark className={LOCKUP_MARK_HEIGHT[orientation]} />
      <span>PumpOS</span>
    </span>
  );
});
